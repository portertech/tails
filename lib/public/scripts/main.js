...
function addStream(s) {
  ...
  var div = $('<div/>',{
    ...
    '<input type="text" class="loggly_token" value="'+s.Forwarding.Token+'" size="45"/>'+
  ...
  $('div#stream_settings_form_'+s.ID+' > input.save_settings').click(function () {
    var stream_name = $('div#stream_settings_form_'+s.ID+' > input.loggly_token').val();
    $.ajax({ url: '/streams/'+s.ID+'/forwarding', type: 'POST', data: { token: stream_name }, timeout: 6000, success: function(data, textStatus, XMLHttpRequest) {
      if (XMLHttpRequest.status == 200) {
        $("#stream_settings_"+s.ID+"[rel]").overlay().close();
      } else {
        // Something seriously went wrong
        $('div#stream_settings_form_'+s.ID+' > div.error').html('Whoops... something went wrong!').show();
        setTimeout(function(){ $('div#stream_settings_form_'+s.ID+' > div.error').hide().html('');}, 3000);
      }
