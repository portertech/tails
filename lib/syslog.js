...
syslog.on('message', function(msgOrig, rinfo) {
  var msg = (/<([^>]+)>([A-Z][a-z]+\s+\d+\s\d+:\d+:\d+) ([^\s]+) (.*)/).exec(msgOrig)
  if (msg) {
    ...
    websocket.sockets.volatile.emit('msg', msgJSON)
    for (var p in logglyPatterns) {
      if (logglyPatterns[p].regex.test(msg[4])) {
        queue.add(loggly(msgJSON, logglyPatterns[p].token))
        break
      }
    }
  }
